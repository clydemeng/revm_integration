...
# BSC Performance & Correctness Verification Script for REVM Integration with Seed Peer
# Tests the performance and correctness of BSC node with REVM integration using local seed peer
...
# Default geth binary (REVM version)
GETH_BINARY="./geth_revm"
...
echo -e "${GREEN}🚀 BSC Node with REVM Integration Test (With Seed Peer)${NC}"
...
echo -e "${BLUE}Phase 1: Performance Testing (REVM Integration)${NC}"
echo "================================================"
...
echo -e "${YELLOW}🚀 Starting BSC node with REVM integration...${NC}"

# Start node with improved peer discovery and REVM integration
nohup "$GETH_BINARY" \
...
echo -e "${BLUE}🔄 Monitoring REVM sync progress to block $TARGET_BLOCKS...${NC}"
if [ -n "$SEED_PEER_ENODE" ]; then
    ...
    if [ "$CURRENT_BLOCK" -gt 0 ]; then
        if [ "$CURRENT_BLOCK" -gt "$LAST_BLOCK" ]; then
            ...
            if [ "$CURRENT_BLOCK" -ge "$WARMUP_BLOCKS" ] && [ "$PERFORMANCE_START_TIME" -eq 0 ]; then
                ...
                echo -e "${GREEN}🏁 Warmup complete! Starting REVM performance measurement...${NC}"
            fi
            ...
            if [ "$CURRENT_BLOCK" -ge "$WARMUP_BLOCKS" ] && [ "$PERFORMANCE_START_TIME" -gt 0 ]; then
                ...
                if [ "$ELAPSED_PERF" -gt 0 ]; then
                    ...
                    echo -e "${GREEN}📦 REVM Block: $CURRENT_BLOCK/$TARGET_BLOCKS | Speed: ${BLOCKS_PER_SECOND} blocks/sec | ETA: $(python3 -c "
...
")${NC}"
                    ...
                    echo -e "${GREEN}📦 REVM Block: $CURRENT_BLOCK/$TARGET_BLOCKS${NC}"
                fi
            else
                echo -e "${YELLOW}🔄 REVM Warmup: $CURRENT_BLOCK/$WARMUP_BLOCKS blocks${NC}"
            fi
        ...
        echo -e "${YELLOW}⏳ Waiting for REVM sync to start...${NC}"
    fi
...
echo -e "${GREEN}🎉 REVM Sync completed!${NC}"
echo -e "${BLUE}📊 REVM Performance Summary:${NC}"
...
for BLOCK_NUM in "${BLOCKS_TO_VERIFY[@]}"; do
    if [ "$BLOCK_NUM" -le "$CURRENT_BLOCK" ]; then
        ...
        if [ "$LOCAL_HASH" = "$MAINNET_HASH" ] && [ "$LOCAL_STATE_ROOT" = "$MAINNET_STATE_ROOT" ]; then
            echo -e "${GREEN}✅ REVM Block $BLOCK_NUM: VERIFIED${NC}"
        else
            echo -e "${RED}❌ REVM Block $BLOCK_NUM: MISMATCH${NC}"
            echo "  REVM hash: $LOCAL_HASH"
            echo "  Mainnet hash: $MAINNET_HASH"
            echo "  REVM state root: $LOCAL_STATE_ROOT"
            echo "  Mainnet state root: $MAINNET_STATE_ROOT"
        fi
    fi
done

echo -e "${GREEN}🎉 REVM performance test completed successfully!${NC}"
if [ -n "$SEED_PEER_ENODE" ]; then
    echo -e "${BLUE}💡 Seed peer helped accelerate the REVM test startup${NC}"
fi 